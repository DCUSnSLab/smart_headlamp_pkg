...
def object_callback(msg: Object):
	...
	angle = JointState()
	angle.header.frame_id = 'zed2i_left_camera_frame'
	angle.header.stamp = rospy.Time.now()
	angle.name = ['joint1', 'joint2']
	
	joint1 = calculate_xy_angle(msg.position[X], msg.position[Y])		# msg.points[0].x, msg.points[0].y
	joint2 = calculate_yz_angle(msg.position[Y], (msg.position[Z]) - 0.11)	# msg.points[0].y, (msg.points[0].z - 0.11)
	...
	pnt = Point()
	pnt.x = msg.position[X]
	pnt.y = msg.position[Y]
	pnt.z = msg.position[Z]
...
if __name__ == '__main__':
	...
	subscriber = rospy.Subscriber('/zed2i/zed_node/obj_det/objects', Object, object_callback) 
	rospy.spin()
